...
func PostGetFlat(db *sql.DB, threadID int, limit int, desc bool, since int) *models.Posts {
	...
	if desc {
		queryStr += " ORDER BY id DESC LIMIT $2"
	} else {
		queryStr += " ORDER BY id LIMIT $2"
	}
...
func PostGetTree(db *sql.DB, threadID int, limit int, desc bool, since int) *models.Posts {
	...
	if desc {
		queryStr += " ORDER BY path DESC, id DESC LIMIT $2"
	} else {
		queryStr += " ORDER BY path, id LIMIT $2"
	}
